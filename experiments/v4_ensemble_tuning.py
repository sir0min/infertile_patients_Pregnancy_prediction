"""
IVF 임신 성공 예측 - 스태킹 솔루션 v4
=======================================
현재 베스트: OOF AUC 0.7402  (solution_v3: LGB+XGB+CatBoost 시드 앙상블)
목표: 스태킹으로 0.7405+ 달성

전략:
  Level 0: XGB × 3 seeds + CatBoost × 3 seeds + LightGBM × 3 seeds = 9 모델
  Level 1: Meta-LightGBM + Meta-Logistic Regression
  Final : Optuna로 Meta-LGB / Meta-LR 최적 가중치 탐색

신규 핵심 피처 (배아 이식 경과일 corr ~0.25):
  - 혼합_이식_간격  : 배아 이식 경과일 - 난자 혼합 경과일
  - 이식일_D5이상   : 배아 이식 경과일 >= 5 (블라스토시스트 이식)
  - 정확히_D5       : 배아 이식 경과일 == 5
  - D5_최적이식_복합: D5 × 이식된 배아 수
  - 이식일x이식수   : 배아 이식 경과일 × 이식된 배아 수
  + 기타 난자/배아 품질, 나이 교호작용 피처

데이터 유출 방지:
  - 모든 통계는 train에서만 계산
  - Meta learner: CV 내부 fold에서만 학습
  - StandardScaler: fold 내부에서만 fit → test에 transform
"""
...
ID_COL  = "ID"
N_FOLDS = 5
SEEDS   = [42, 777, 2024]   # Level-0 base model 시드 (3개)
SEED    = 42


# ══════════════════════════════════════════════════════════════════════════════
# 1. 피처 엔지니어링 (v3 + 신규 29개)
# ══════════════════════════════════════════════════════════════════════════════
...
def feature_engineering_v4(df: pd.DataFrame, fe_stats: dict = None) -> tuple:
    """
    v3 파생변수(27개) + 신규 피처(배아 이식 경과일 corr ~0.25 포함)
    """
    df = df.copy()
    ...
    # 이식 경과일 (핵심 신규 피처 소스, corr ~0.25)
    et_day  = df["배아 이식 경과일"]     # 배아 이식 경과일
    mix_day = df["난자 혼합 경과일"]     # 난자 혼합 경과일

    # ── 순서형 인코딩 열: 계산용 clip / 플래그용 원본 ─────────────────────────
    ...
    # ── 그룹 A: 배아 이식 경과일 (corr ~0.208~0.252) ─────────────────────────
    # Day 5 이식(블라스토시스트) = IVF 성공률 가장 높음
...
# ══════════════════════════════════════════════════════════════════════════════
# 2. Level-0 베이스 모델 학습 (3 seeds × 3 모델 = 9 모델)
# ══════════════════════════════════════════════════════════════════════════════

# 검증된 파라미터 (solution_v3 Optuna 탐색 결과 기반)
LGB_PARAMS = {
...
def train_base_models(X_train, X_test, y):
    """
    9개 베이스 모델 학습 후 OOF / Test 예측 반환.
    반환: all_oof dict, all_pred dict
    """
    ...
    # 9모델 단순 평균 앙상블
    baseline_oof  = np.mean(list(all_oof.values()), axis=0)
    baseline_pred = np.mean(list(all_pred.values()), axis=0)
    print(f"\n  베이스라인 (seed 앙상블) OOF AUC: {roc_auc_score(y, baseline_oof):.5f}")

    return all_oof, all_pred, baseline_oof, baseline_pred
...
def train_meta_learners(all_oof, all_pred, y):
    """
    Level-0 OOF를 입력으로 Meta-LGB + Meta-LR 학습.
    반환: oof_meta_lgb, oof_meta_lr, pred_meta_lgb, pred_meta_lr
    """
    ...
    # ── Meta-LightGBM ──────────────────────────────────────────────────────
    print("\n  Meta-LGB 학습...")
    oof_meta_lgb  = np.zeros(n_tr)
    pred_meta_lgb = np.zeros(n_te)
    for fold, (tr_i, val_i) in enumerate(cv.split(meta_train, y)):
        m = lgb.LGBMClassifier(**META_LGB_PARAMS)
        m.fit(meta_train[tr_i], y[tr_i],
              eval_set=[(meta_train[val_i], y[val_i])],
              callbacks=[lgb.early_stopping(50, verbose=False),
                         lgb.log_evaluation(-1)])
        oof_meta_lgb[val_i]  = m.predict_proba(meta_train[val_i])[:, 1]
        pred_meta_lgb       += m.predict_proba(meta_test)[:, 1] / N_FOLDS
    print(f"  Meta-LGB OOF AUC: {roc_auc_score(y, oof_meta_lgb):.5f}")

    # ── Meta-Logistic Regression (fold 내부 스케일링) ─────────────────────
    print("\n  Meta-LR 학습...")
    oof_meta_lr  = np.zeros(n_tr)
    pred_meta_lr = np.zeros(n_te)
    for fold, (tr_i, val_i) in enumerate(cv.split(meta_train, y)):
        sc  = StandardScaler()
        mtr = sc.fit_transform(meta_train[tr_i])   # train fold에서만 fit
        mva = sc.transform(meta_train[val_i])
        mte = sc.transform(meta_test)

        lr = LogisticRegression(max_iter=1000, C=1.0, random_state=SEED)
        lr.fit(mtr, y[tr_i])
        oof_meta_lr[val_i]  = lr.predict_proba(mva)[:, 1]
        pred_meta_lr       += lr.predict_proba(mte)[:, 1] / N_FOLDS
    print(f"  Meta-LR  OOF AUC: {roc_auc_score(y, oof_meta_lr):.5f}")

    return oof_meta_lgb, oof_meta_lr, pred_meta_lgb, pred_meta_lr


# ══════════════════════════════════════════════════════════════════════════════
# 4. 최종 블렌딩 가중치 Optuna 최적화
# ══════════════════════════════════════════════════════════════════════════════

def optimize_final_blend(oof_meta_lgb, oof_meta_lr, y):
    def objective(trial):
        w = trial.suggest_float("w_lgb", 0.0, 1.0)
        oof = w * oof_meta_lgb + (1 - w) * oof_meta_lr
    ...
    print(f"\n  최적 가중치: Meta-LGB={w_lgb:.3f}  Meta-LR={1-w_lgb:.3f}")
    print(f"  스태킹 최종 OOF AUC: {best_auc:.5f}")
    return w_lgb, best_auc
...
def print_comparison(scores: dict):
    ...
    for name, auc in sorted(scores.items(), key=lambda x: -x[1]):
        ...
        print(f"  {name:<30} {auc:.5f}  {bar}{star}")
    print("=" * 60)
...
def main(data_dir="data", output_path="submission_v4.csv"):
    print("=" * 60)
    print(" IVF 임신 성공 예측 - Solution v4 (2-Level Stacking)")
    print("=" * 60)
    ...
    # ── Level-0 베이스 모델 (9개) ────────────────────────────────────────────
    print(f"\n[3] Level-0 베이스 모델 학습 ({len(SEEDS)} seeds × 3 모델 = {len(SEEDS)*3}개)")
    all_oof, all_pred, base_oof, base_pred = train_base_models(X_train, X_test, y)

    # ── Level-1 메타 러너 ────────────────────────────────────────────────────
    print("\n[4] Level-1 메타 러너 학습")
    oof_lgb, oof_lr, pred_lgb, pred_lr = train_meta_learners(all_oof, all_pred, y)

    # ── 최적 블렌딩 ──────────────────────────────────────────────────────────
    print("\n[5] 최적 블렌딩 가중치 탐색 (Optuna 200 trials)")
    w_lgb, stacking_auc = optimize_final_blend(oof_lgb, oof_lr, y)

    # ── 성능 비교 ─────────────────────────────────────────────────────────────
    scores = {
        "스태킹 최종":          stacking_auc,
        "베이스라인 (seed 앙블)": roc_auc_score(y, base_oof),
        "Meta-LGB":             roc_auc_score(y, oof_lgb),
        "Meta-LR":              roc_auc_score(y, oof_lr),
    }
    # 개별 모델 점수 추가
    ...
    # ── 제출 파일 생성 ────────────────────────────────────────────────────────
    final_preds = w_lgb * pred_lgb + (1 - w_lgb) * pred_lr
