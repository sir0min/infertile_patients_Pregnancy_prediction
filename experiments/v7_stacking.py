"""
IVF 임신 성공 예측 - Solution v7 (TabNet with Categorical Embeddings)
=======================================================================
v6 -> v7 핵심 개선:
  TabNet 성능 부족 문제 해결:
  1. 카테고리 임베딩 (cat_idxs + cat_dims + cat_emb_dim=3)
     - 순서형 인코딩된 열 (나이, 시술횟수, 등) → 임베딩 벡터로 표현
     - -1 (알 수 없음) → 비음수 정수로 시프트
  2. 더 큰 모델: n_d=n_a=32, n_steps=5
  3. 더 긴 학습: max_epochs=300, patience=40
  4. batch_size=4096 (더 작은 배치 → 더 나은 일반화)
  5. 3 seeds (42, 777, 2024)

예상 TabNet OOF: 0.735+ (v6: 0.730)
목표: 전체 스태킹 OOF AUC 0.7405+
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# 2. TabNet용 카테고리 피처 준비
# ══════════════════════════════════════════════════════════════════════════════

def prepare_tabnet_features(X_train_df: pd.DataFrame,
                             X_test_df: pd.DataFrame,
                             max_cat_cardinality: int = 15):
    """
    정수형 저-기수성 열 → 카테고리 임베딩용 비음수 정수로 변환.
    연속형 열은 StandardScaler로 정규화.
    반환: (X_tr_np, X_te_np, cat_idxs, cat_dims)
    """
    ...
    for i, col in enumerate(X_train_df.columns):
        ...
        if n_unique <= max_cat_cardinality:
            ...
            if min_val < 0:
                shift = int(-min_val)
                X_tr[col] = X_tr[col] + shift
                X_te[col] = X_te[col] + shift
            cat_idxs.append(i)
            cat_dims.append(int(X_tr[col].max()) + 1)
        else:
            cont_idxs.append(i)

    # 연속형 열만 StandardScaler 정규화
...
# ══════════════════════════════════════════════════════════════════════════════
# 4. Level-0 TabNet (카테고리 임베딩 + 큰 모델)
# ══════════════════════════════════════════════════════════════════════════════

def train_tabnet_models(X_tr_np, X_te_np, y, cat_idxs, cat_dims):
    """
    n_d=32, n_a=32, n_steps=5, cat_emb_dim=3, max_epochs=300
    """
...
def optimize_4class_blend(tree_oof, tree_pred, tab_oof, tab_pred, y):
    ...
    print(f"  4-class 블렌드 OOF AUC: {auc:.5f}")
    return opt_oof, opt_pred, auc
...
def train_meta_lr(all_oof, all_pred, y):
    ...
    print(f"  Meta-LR ({meta_tr.shape[1]} OOF) AUC: {auc:.5f}")
...
def main(data_dir="data", output_path="submission_v7.csv"):
    print("=" * 60)
    print(" IVF 임신 성공 예측 - Solution v7 (TabNet Cat-Emb)")
    print("=" * 60)
    ...
    # TabNet 입력 준비 (카테고리 임베딩 + 연속형 스케일링)
    print("\n[3] TabNet 피처 준비 (카테고리 임베딩)")
    X_tr_np, X_te_np, cat_idxs, cat_dims = prepare_tabnet_features(
        X_train, X_test, max_cat_cardinality=15
    )

    # Level-0: 트리 모델 9개
    print(f"\n[4] 트리 모델 학습 (3 seeds x 3 = 9개)")
    tree_oof, tree_pred, tree_avg_oof, tree_avg_pred = train_tree_models(
        X_train, X_test, y
    )

    # Level-0: TabNet 3개 (카테고리 임베딩, n_d=32)
    print(f"\n[5] TabNet 학습 ({len(TAB_SEEDS)} seeds, device={DEVICE}, n_d=32, cat_emb=3)")
    tab_oof, tab_pred, tab_avg_oof, tab_avg_pred = train_tabnet_models(
        X_tr_np, X_te_np, y, cat_idxs, cat_dims
    )

    # 4-class 최적 블렌드
    print(f"\n[6] 4-class 최적 가중치 탐색 (Optuna 500)")
    opt_oof, opt_pred, opt_auc = optimize_4class_blend(
        tree_oof, tree_pred, tab_oof, tab_pred, y
    )

    # Meta-LR (전체 12개 OOF)
    print(f"\n[7] Meta-LR 학습 (전체 12개 OOF)")
    ...
    scores = {
        ...
        "Meta-LR (12 OOF)":     lr_auc,
        "트리 단순 평균":       roc_auc_score(y, tree_avg_oof),
        "TAB 평균 (cat-emb)":   roc_auc_score(y, tab_avg_oof),
    }
