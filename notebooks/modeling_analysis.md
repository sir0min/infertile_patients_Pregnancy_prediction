...
`model_comparison.py`(15:54)에서 LogisticRegression, RandomForest, GradientBoosting, LightGBM, CatBoost 등 여러 모델의 5-fold OOF AUC를 비교하며 GBDT 계열이 압도적으로 우수함을 확인했습니다. 이어 `ensemble_optuna.py`(17:56)에서 LGB·CAT에 Optuna 하이퍼파라미터 탐색을 적용하고, `final_model.py`(19:57)에서 XGBoost를 추가한 LGB·XGB·CAT 3-way 앙상블로 발전시켰습니다. `advanced_model.py`(22:06)에서는 ExtraTrees, HistGradientBoosting 등 추가 모델을 실험했으나 GBDT 3종 대비 유의미한 향상이 없어 이후 버전에서 제외했습니다.

---

### 4월 27일 — 시드 앙상블·스태킹 체계 도입 및 피처 엔지니어링 확장

`solution_v3.py`(17:52)에서 Optuna로 최적화된 LGB·XGB·CAT를 4개 시드로 반복 학습한 후 가중치 최적화 앙상블을 적용하여 OOF AUC **0.74020**을 달성했습니다(LGB=0.05, XGB=0.40, CAT=0.55). `solution_v4.py`(19:18)에서는 피처 엔지니어링을 27개에서 122개로 대폭 확장하고, 3 seed × 3 모델 = 9개 기본 모델에 Meta-LGB + Meta-LR 2단계 스태킹을 도입했습니다. 그러나 Meta-LGB는 과적합(0.73965)하고 Meta-LR이 단순 평균보다 낮은 **0.74015**를 기록해, 스태킹이 동일 계열(GBDT) 모델만으로는 효과가 없다는 것을 확인했습니다.

---

### 4월 28일 — TabNet 도입, 카테고리 임베딩 최적화, 추가 실험

`solution_v5.py`(00:38)에서 Meta-LGB를 제거하고 Optuna로 LGB·XGB·CAT의 3차원 최적 가중치를 탐색하여 OOF **0.74023**으로 반등했습니다. `solution_v6.py`(00:59)에서 모델 다양성 확보를 위해 TabNet을 최초 추가했으나, 카테고리 임베딩 없는 기본 구성(n_d=16)으로는 TAB avg OOF 0.73338에 불과해 블렌딩 기여가 0.4%에 그쳤습니다. `solution_v7.py`(03:38)에서 TabNet에 카테고리 임베딩(cat_idxs, cat_dims, cat_emb_dim=3)을 적용하고 모델 크기를 n_d=32로 키우자 TAB avg OOF가 0.73689로 향상되며 기여도 10.3%로 증가, 최종 OOF **0.74027**로 전체 최고 기록을 달성했습니다. 이후 `solution_v8.py`(11:47)에서 CatBoost depth를 7→8로 키웠으나 오히려 0.74025로 소폭 하락했고, `solution_v9.py`(21:31)에서 중요도=0 피처 14개와 상관계수 0.97+ 중복 피처 5개를 제거해 122→103개로 줄였지만 0.74023으로 역시 하락했습니다. 제거된 피처들도 앙상블 내에서 소량의 신호를 제공하고 있었음을 확인한 결과입니다.

---

**현재 제출 최고: `submission_v7.csv` (OOF AUC 0.74027)**

| 버전 | 최종 OOF AUC | 트리 평균 | TabNet | 핵심 변경 |
| --- | --- | --- | --- | --- |
| **v3** | 0.74020 | 0.74020 | - | Optuna 단일 모델 튜닝 + 3-way 블렌드 (시드 앙상블 없음) |
| **v4** | 0.74015 | 0.74018 | - | 시드 앙상블(3×3=9개) + Meta-LR 스태킹 도입, 피처 122개 |
| **v5** | 0.74023 | 0.74017 | - | 4-class Optuna 블렌드 (CAT 비중↑), Meta-LGB 과적합 제거 |
| **v6** | 0.74023 | 0.74017 | 0.73338 | TabNet 최초 추가 (임베딩 없음, n_d=16) → 기여 0.4%에 불과 |
| **v7 ★** | **0.74027** | 0.74017 | 0.73689 | TabNet 카테고리 임베딩 (n_d=32), 기여 10.3%로 증가 |
| **v8** | 0.74025 | 0.74016 | 0.73670 | CatBoost depth 7→8 (역효과, -0.00002) |
| **v9** | 0.74023 | 0.74011 | 0.73662 | 피처 122→103개 제거 (노이즈/중복, 역효과, -0.00004) |
